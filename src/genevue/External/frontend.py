...
def gffread_cds_pep_extract(
    ...
    Args:
        gff3_file_path: SimpleGFF3 file path.
    ...
    try:
        subprocess.run(
            cmd_cds, check=True, capture_output=True, text=True, cwd=str(workdir)
        )
        subprocess.run(
            cmd_pep, check=True, capture_output=True, text=True, cwd=str(workdir)
        )
    ...
    console.info(f"Outputs of gffread are saved in {workdir}.")
    ...
    return (
        {cds.id: cds for cds in SeqIO.parse(str(out_cds_path), "fasta")},
        {pep.id: pep for pep in SeqIO.parse(str(out_pep_path), "fasta")},
    )


def muscle5(
    pep_file_path: str, workdir: str, muscle5_cmd: str = "muscle"
) -> dict[str, SeqRecord]:
    subprocess.run(
...
def mafft(workdir: str, pep_file_name: str, res_file_name: str) -> None:
