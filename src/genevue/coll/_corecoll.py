...
class CollRes:
    ...
    def get_block(self, bkidx: int) -> tuple[pd.DataFrame, float, float]:
        return (
            self._total_block[self._total_block["bkidx"] == bkidx].loc[
                :, ["loc1", "loc2"]
            ],
...
class DoCollinearity:
    ...
    def get_matrix(self):
        ...
        self.points["path1"] = (
            self.points.index.to_numpy().reshape(len(self.points), 1).tolist()
        )
    ...
    def run(self):
        ...
        points1 = self.points[
            ["loc1", "loc2", "score1", "path1", "usedtimes1"]
        ...
        points2 = self.points[
            ["loc1", "loc2", "score2", "path2", "usedtimes2"]
        ...
        for index, row, col in self.points[["loc1", "loc2"]].itertuples():
            ...
            for index_ij, row_i, col_j, grading in points[
                ...
                if score > 0 and self.points.at[index_ij, "score1"] < score1:
                    ...
                    self.points.at[index_ij, "path1"] = self.points.at[
                        index, "path1"
                    ] + [index_ij]
        ...
        for index, row, col in points_reverse[["loc1", "loc2"]].itertuples():
            ...
            for index_ij, row_i, col_j, grading in points[
                ...
                if score > 0 and self.points.at[index_ij, "score2"] < score2:
                    ...
                    self.points.at[index_ij, "path2"] = self.points.at[
                        index, "path2"
                    ] + [index_ij]
                    gap = min(col_j - col, gap)
                    row_i_old = row_i
    ...
    def max_path(self, points):
        ...
        if (
            self.over_length >= self.over_gap
            and len(self.path) / self.over_length > self.coverage_ratio
        ):
            points.drop(index=self.path.index, inplace=True)
            [loc1_min, loc2_min], [loc1_max, loc2_max] = (
                self.path[["loc1", "loc2"]].agg(["min", "max"]).to_numpy()
            )

            # Calculate p-value
            gap_init = self.points_init[
                (loc1_min <= self.points_init["loc1"])
                & (self.points_init["loc1"] <= loc1_max)
                & (loc2_min <= self.points_init["loc2"])
                & (self.points_init["loc2"] <= loc2_max)
            ].copy()

            self.p_value = self.p_value_estimated(
