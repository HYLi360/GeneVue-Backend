        ...
        def wrapped_func(*args, **kwargs):
            try:
                if "{" in self.msg and self.on_func_exit:
                    ...
                    self.retval = func(*args, **kwargs)
                return self.retval
            except (BuscoError, BatchFatalError, BuscoConfigError):
                raise
...
class BuscoLogger(logging.getLoggerClass()):
    ...
    def __init__(self, name):
        ...
        # Console output is handled by the genevue.QC.BUSCO parent logger
        # (RichHandler), so we only add the in-memory warning capture here.
        # The file handler is created lazily via init_file_handler() when an
        # actual BUSCO run starts.
    ...
    def _log(self, *args, **kwargs):
        if type(self).quiet:
            if not type(self).quiet_msg_logged:
                ...
                # Suppress INFO/WARNING on the Rich console handler
                busco_parent = logging.getLogger("genevue.QC.BUSCO")
                for h in busco_parent.handlers:
    ...
    @classmethod
    def init_file_handler(cls, log_filename=None):
        """Create the BUSCO debug log file handler.

        Must be called explicitly when an actual BUSCO run starts.
        Uses ppid/pid logic so child processes log to the parent's file.
        """
        if cls._file_hdlr is not None:
            return
        if log_filename is None:
            log_filename = "busco_{}.log".format(cls.ppid)
            if not os.path.exists(log_filename):
                log_filename = "busco_{}.log".format(cls.pid)
        try:
            cls._file_hdlr = logging.FileHandler(log_filename, mode="a")
        except IOError as e:
            errStr = (
        ...
        cls._file_hdlr.setLevel(logging.DEBUG)
        cls._file_hdlr.setFormatter(
            logging.Formatter(
                "%(asctime)s %(levelname)s:%(name)s\t%(message)s",
                datefmt="%Y-%m-%d %H:%M:%S",
            )
        )
        logging.getLogger("genevue.QC.BUSCO").addHandler(cls._file_hdlr)

    @classmethod
    def close_file_handler(cls):
        """Remove and close the shared file handler."""
        if cls._file_hdlr is not None:
            busco_parent = logging.getLogger("genevue.QC.BUSCO")
            busco_parent.removeHandler(cls._file_hdlr)
            cls._file_hdlr.close()
            cls._file_hdlr = None
