...
class BuscoDownloadManager:
    ...
    def check_latest_version(self, data_name):
        if type(self).version_files is not None and not type(self).version_files.empty:
            if data_name in type(self).version_files.index:
                latest_update = type(self).version_files.loc[data_name]["date"]
                return latest_update
            else:
                raise BuscoError("{} is not a valid download option".format(data_name))
        else:
    ...
    def get(self, data_name, category):
        ...
        elif up_to_date and present:
            logger.info(
                "The local file or folder {} is the last available version.".format(
                    local_filepath
                )
            )
