...
from genevue.QC.BUSCO.Analysis import NucleotideAnalysis, BLASTAnalysis
from genevue.QC.BUSCO.prodigal import ProdigalRunner
from genevue.QC.BUSCO.metaeuk import MetaeukRunner
from genevue.QC.BUSCO.miniprot import MiniprotIndexRunner, MiniprotAlignRunner
from genevue.QC.BUSCO.bbtools import BBToolsRunner
from genevue.QC.BUSCO.augustus import (
    AugustusRunner,
    GFF2GBRunner,
    NewSpeciesRunner,
    ETrainingRunner,
...
from genevue.QC.BUSCO.base import NoRerunFile, NoGenesError
...
from genevue.QC.BUSCO.Exceptions import BuscoError
import numpy as np
import re
...
class GenomeAnalysisEukaryotes(GenomeAnalysis):
    ...
    def trim_sequence(self, old_gene_match, new_gene_match, start_trim, end_trim):
        ...
        try:
            ...
            new_sequence_nt.id = new_sequence_nt.name = new_sequence_nt.description = (
                new_gene_match
            )
        except KeyError:
        ...
        new_sequence_aa.id = new_sequence_aa.name = new_sequence_aa.description = (
            new_gene_match
        )
...
class GenomeAnalysisEukaryotesAugustus(BLASTAnalysis, GenomeAnalysisEukaryotes):
    ...
    def cleanup(self):
        try:
            ...
            pass
        try:  # Remove tmp dir created by gff2gb - this is the same as the tmp dir created by augustus for individual err logs, deleted below
            self.gff2gb_runner.remove_tmp_dir()
        except OSError:
            pass
        try:
            self.augustus_runner.remove_individual_err_logs()
        except OSError:
            pass
        super().cleanup()
            ...
            with open(self.miniprot_align_runner.output_gff, "r") as g:
                ...
                for busco_id, match_dict in result_dict.items():
                    ...
                    for gene_id, details in match_dict.items():
                        ...
                        g.seek(gff_start)
                        while g.tell() < gff_end:
                            extracted_gff_lines.append(g.readline())
