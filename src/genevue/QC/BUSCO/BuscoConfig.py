...
from genevue.QC.BUSCO.Exceptions import BuscoConfigError
...
class BaseConfig(ConfigParser, metaclass=ABCMeta):
        ...
        if "genome" in mode:
            if domain in ["prokaryota", "viruses"]:
                if self.getboolean("busco_run", "use_miniprot"):
                    if self.get("busco_run", "datasets_version") != "odb10" and not (
                        os.path.exists(
                            os.path.join(
                                self.get("busco_run", "lineage_dataset"),
                                "refseq_db.faa.gz",
                            )
                        )
                        or os.path.exists(
                            os.path.join(
                                self.get("busco_run", "lineage_dataset"),
                                "refseq_db.faa",  # possible the refseq file was decompressed on an earlier run
                            )
                        )
                    ):
                        raise BuscoConfigError(
                ...
                raise BuscoConfigError("Unrecognized mode {}".format(mode))
        ...
        try:
            with open(
                ...
                if "_odb" in dataset_kwargs["name"]:
                    ...
                    self.set("busco_run", "datasets_version", odb_version)
            ...
            raise BuscoConfigError("Dataset configuration file is not valid")
    ...
    def _load_config_file(self, conf_file):
        ...
        try:
            ...
            raise BuscoConfigError("Config file {} cannot be found".format(conf_file))
        except ParsingError:
            raise BuscoConfigError(
...
class PseudoConfigForPlot(PseudoConfig):
    def __init__(self, conf_file, params):
        super().__init__(conf_file, params)
    ...
    def _fill_default_values(self):
        try:
            self.get("busco_run", "plot_percentages")
        except NoOptionError:
            self.set(
                "busco_run",
                "plot_percentages",
                type(self).DEFAULT_ARGS_VALUES["plot_percentages"],
            )


class PseudoConfigForDownload(PseudoConfig):
    def __init__(self, conf_file, params):
        super().__init__(conf_file, params)
...
class BuscoConfigMain(BaseConfig):
    ...
    def _check_limit_value(self):
        """
        Check the value of limit. Ensure it is between 1 and 20, otherwise raise BuscoConfigError.
        :return:
        """
