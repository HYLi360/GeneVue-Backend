...
class AutoSelectLineage:
    ...
    @log(
        ...
        "However, a high BUSCO score in multiple domains might help you identify possible contamination.",
        logger,
    )
    ...
    def run_auto_selector(self):
        ...
        type(self).root_dataset_versions = [
            "{}_{}".format(self.all_lineages[i], odb_versions[i])
            for i in range(len(self.all_lineages))
        ]
        root_runners = self.run_lineages_list(self.all_lineages, versions=odb_versions)
        self.get_best_match_lineage(root_runners, use_percent=True)
        if self.virus_check() and not self.config.getboolean(
            ...
            self.get_best_match_lineage(type(self).runners, use_percent=True)
                ...
                try:
                    ...
                    if "not a valid download option" in str(e):
                        logger.warning(
                            ...
                            )  # this code no longer used - it was to handle the slight delay in the release of placement files for the odb12 datasets
                        )
