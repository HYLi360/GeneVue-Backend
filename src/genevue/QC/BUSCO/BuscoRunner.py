...
from genevue.QC.BUSCO.BuscoConfig import BuscoConfigMain
from genevue.QC.BUSCO.BuscoLogger import BuscoLogger
from genevue.QC.BUSCO.BuscoLogger import LogDecorator as log
from genevue.QC.BUSCO.Exceptions import BatchFatalError, BuscoError
from genevue.QC.BUSCO.GeneSetAnalysis import GeneSetAnalysis
from genevue.QC.BUSCO.GenomeAnalysis import (
    GenomeAnalysisEukaryotesAugustus,
    GenomeAnalysisEukaryotesMetaeuk,
    GenomeAnalysisEukaryotesMiniprot,
    GenomeAnalysisProkaryotes,
)
from genevue.QC.BUSCO.TranscriptomeAnalysis import (
    TranscriptomeAnalysisProkaryotes,
    TranscriptomeAnalysisEukaryotes,
)
from genevue.QC.BUSCO.base import NoGenesError, BaseRunner
...
class SingleRunner:
    ...
    def auto_select_lineage(self):
        from genevue.QC.BUSCO.AutoLineage import (
            AutoSelectLineage,
    ...
    def run(self):
        try:
            ...
            try:
                input_file_checksum, input_file_size = self.get_input_checksum()

                if self.runner is not None:
                    try:
                        self.run_stats[input_file_checksum] = AnalysisRunner.run_stats[
                            "results"
                        ]
                        self.run_stats[input_file_checksum].update(
                            {
                                "input_file_size": input_file_size,
                            }
                        )
                        self.run_stats[input_file_checksum].update(
                            {"versions": AnalysisRunner.run_stats["versions"]}
                        )
                        self.run_stats["timings"].update(
                            {input_file_checksum: AnalysisRunner.run_stats["timings"]}
                        )
                        self.run_stats[input_file_checksum].update(
    ...
    def get_input_checksum(self):
        ...
        try:
            ...
            file_size = os.stat(self.config.get("busco_run", "in")).st_size
        except:
            pass

        return md5_checksum, file_size
...
class BatchRunner:
    ...
    def __init__(self, config_manager):
        ...
        self.run_stats = {"num_batch_inputs": self.num_inputs, "results": {}}
        self.gc_selected = False
        self.include_miniprot_summary_params = False
    ...
    def run(self):
        for i, input_file in enumerate(self.input_files):
            try:
                ...
                try:
                    if (
                        single_run.runner.analysis.prodigal_runner is not None
                    ):  # add translation table if prodigal was used
                        self.gc_selected = True
                except AttributeError:
                    pass

                run_summary = single_run.runner.format_run_summary()
                type(self).batch_results.append(run_summary)
                type(self).auto_lineage_dataset_versions = (
                    single_run.auto_lineage_dataset_versions
                )
                self.run_stats["results"].update(single_run.run_stats)

                if self.config.getboolean(
                    "busco_run", "use_miniprot"
                ):  # set parameter in BatchRunner as it is reset in the config
                    self.include_miniprot_summary_params = True

                if not self.config.params[
                    "use_miniprot"
                ]:  # reset the use_miniprot flag if it was not set in the command line params
                    self.config.set("busco_run", "use_miniprot", "False")
    ...
    def write_batch_summary(self):
        ...
        with open(summary_file, "w") as f:
            if "genome" in self.config.get("busco_run", "mode"):
                ...
                if self.include_miniprot_summary_params:
            ...
            if self.config.getboolean("busco_run", "auto-lineage"):
                ...
                bacteria_version = "bacteria_{}".format(
                    BuscoConfigMain.DEFAULT_ARGS_VALUES["datasets_version"]
                )
...
class AnalysisRunner:
    ...
    def save_results(self, exception_raised=False):
        ...
        try:
            self.final_results[self.lineage_basename] = (
            ...
            if self.analysis.hmmer_runner.avg_identity is not None:
                self.all_results[self.lineage_basename][
                    "avg_identity"
            ...
            try:
                if self.analysis.prodigal_runner.current_gc is not None:
                    self.all_results[self.lineage_basename][
                        "translation_table"
                    ] = self.analysis.prodigal_runner.current_gc
            except AttributeError:
                # If prodigal was not used, this attribute will not exist
                pass
    ...
    def compile_summary(self):
        ...
        try:
            type(self).run_stats["versions"].update(BaseRunner.tool_versions)
            self.summary["versions"] = type(self).run_stats["versions"]
            type(self).run_stats["timings"] = BaseRunner.run_stats
        except:
            pass
        ...
        try:
            ...
            if "results" in type(self).run_stats:
                type(self).run_stats["results"].update(
                    {
                        "{}.{}".format(
                            self.lineage_basename, self.dataset_creation_date
                        ): type(self).all_results[self.lineage_basename]
                    }
                )
            else:
                type(self).run_stats["results"] = {
                    "{}.{}".format(
                        self.lineage_basename, self.dataset_creation_date
                    ): type(self).all_results[self.lineage_basename]
                }
        ...
        with open(self.short_summary_file, "w") as summary_file:
            ...
            if "genome" in self.mode:
                ...
                for key, val in self.analysis.bbtools_runner.metrics.items():
                    if "N50" in key:
                        if int(val) > 1000000:
                            val = "{} Mbp".format(int(val) // 1000000)
                        elif int(val) > 1000:
                            val = "{} kbp".format(int(val) // 1000)
                    summary_file.write("\t{}\t{}\n".format(val, key))
                summary_file.write("\n")
    ...
    def format_run_summary(self):
        ...
        try:
            extra += "\t{}".format(self.analysis.prodigal_runner.current_gc)
        except AttributeError:
            pass
        ...
        if self.config.getboolean("busco_run", "auto-lineage"):
            ...
            try:
                scores_bact = (
                    type(self).all_results[bacteria_version]["one_line_summary"].strip()
                )
            except KeyError:
                scores_bact = "Not run"
            try:
    ...
    def backup_log_file(config):
        try:
            ...
            if os.path.exists(os.path.join(log_folder, "BUSCO.log")):
                timestamp = datetime.now().strftime("%Y-%m-%d.%H:%M:%S")
                shutil.move(
                    os.path.join(log_folder, "BUSCO.log"),
                    os.path.join(log_folder, "BUSCO.backup.{}.log".format(timestamp)),
                )

        except OSError:
            logger.warning(
                "Unable to backup 'BUSCO.log' to 'BUSCO.backup.<timestamp>.log'."
            )
        return
