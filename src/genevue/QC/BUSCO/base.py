...
from genevue.QC.BUSCO.Toolset import Tool
from shutil import which
from abc import ABCMeta, abstractmethod
from genevue.QC.BUSCO.BuscoConfig import BuscoConfigAuto
...
class BaseRunner(Tool, metaclass=ABCMeta):
    ...
    def __init__(self):
        ...
        self.tmp_dir = os.path.join(self.main_out, "tmp")
    ...
    def write_checkpoint_file(self, additional_args=[]):
        with open(self.checkpoint_file, "a") as cpt_file:
            ...
            cpt_file.write("Time: {}\n".format(current_time))
            type(self).run_stats["{}_start_time".format(self.name)] = current_time
...
class GenePredictor(BaseRunner, metaclass=ABCMeta):
    ...
    def decompress_refseq_file(
        ...
        )  # create tmp uncompressed file in a writable location
        tmp_file = os.path.join(self.tmp_dir, os.path.basename(unzipped_filename))
        if not os.path.exists(unzipped_filename):
            with gzip.open(gzip_file, "rb") as compressed_file:
                with open(tmp_file, "wb") as decompressed_file:
                    for line in compressed_file:
                        decompressed_file.write(line)
        else:
            try:
                if os.path.islink(tmp_file) or os.path.exists(tmp_file):
                    os.remove(tmp_file)
                os.symlink(unzipped_filename, tmp_file)
            except OSError as e:
                logger.warning(
                    f"Unable to create symlink {tmp_file} -> {unzipped_filename}: {e}"
                )
