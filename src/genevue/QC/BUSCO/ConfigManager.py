...
from genevue.QC.BUSCO.BuscoConfig import BuscoConfigMain, BuscoConfigAuto
...
class BuscoConfigManager:
    ...
    def load_busco_config_main(self):
        try:
            ...
            self.run_stats.update(self.config_main.run_stats)
        except:
            if self.config_main is not None:
                self.run_stats.update(self.config_main.run_stats)
            raise
        return
