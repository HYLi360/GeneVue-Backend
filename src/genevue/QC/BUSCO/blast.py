...
class MKBLASTRunner(BaseRunner):
    ...
    def configure_job(self, *args):
        ...
        if self.input_file.endswith(".gz"):
            # For gzipped files, pipe through stdin
            mkblast_job.add_parameter("-title")
            mkblast_job.add_parameter(self.input_file)
            mkblast_job.add_parameter("-in")
            mkblast_job.add_parameter("-")
            # Read and decompress the gzipped file
            with gzip.open(self.input_file, "rb") as f:
                mkblast_job.stdin = f.read()
        else:
            # For regular files, pass the filename
            mkblast_job.add_parameter("-in")
            mkblast_job.add_parameter(self.input_file)
...
class TBLASTNRunner(BaseRunner):
    ...
    def _define_flank(self):
        try:
            ...
            IOError
        ):  # Input data is only validated during run_analysis. This will catch any IO issues before that.
            raise BuscoError(
