...
from genevue.QC.BUSCO.base import BaseRunner, GenePredictor, NoGenesError
import os
import re
...
class AugustusRunner(GenePredictor):
    ...
    def remove_individual_err_logs(self):
...
class GFF2GBRunner(BaseRunner):
    ...
    def __init__(self):
        ...
        if self.input_file.endswith(".gz"):
            self.input_file = self.decompress_input_file()

    def decompress_input_file(self):
        """
        Decompress the input file if it is gzipped.
        :return: Path to the decompressed file.
        """

        os.makedirs(self.tmp_dir, exist_ok=True)
        decompressed_file = os.path.join(
            self.tmp_dir, os.path.basename(self.input_file).rstrip(".gz")
        )
        with open(decompressed_file, "wb") as out_f:
            with gzip.open(self.input_file, "rb") as in_f:
                for line in in_f:
                    out_f.write(line)
    ...
    def remove_tmp_dir(self):
        """
        Remove the temporary directory created for decompression.
        """
