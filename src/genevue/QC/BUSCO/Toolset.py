        ...
        with open(self.job_outlogger, "wb") as f_out:
            with open(self.job_errlogger, "wb") as f_err:
                try:
                    ...
                    if "Invalid block no" in err_msg:
                        logger.debug(
                            "The following job failed with the error code {}:\n{}\nThis is a minor augustus bug and not considered fatal".format(
                                e.returncode, " ".join(self.cmd_line)
                            )
                        )
                        logger.debug("Error message: {}".format(err_msg))
                    else:
                        logger.error(
                            "The following job failed with the error code {}:\n{}".format(
                                e.returncode, " ".join(self.cmd_line)
                        ...
                        logger.error("Error message: {}".format(err_msg))

                        raise BuscoError(
                            "Job failed with error code {}".format(e.returncode)
                        )
